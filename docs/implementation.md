...
| D-044 | Locally authored and imported plugins are in scope from the first cut, superseding D-039               | Accepted                         | D-039 held the capability back until the manifest had survived a few real plugins, and the reasoning was sound: a plugin that is really a fragment of Telegraf config is arbitrary user-authored collector configuration, and the renderer's output allowlist (D-034) is what stands between a definition and command execution on the collector host. The owner's authoring, upload, and download scope makes user-authored plugins reachable on day one, so that reasoning no longer describes the product being built. Recorded as superseded rather than quietly widened, because a decision that says one thing while the roadmap says another is worse than no decision. What replaces the delay is a boundary: a manifest is declarative data validated against a strict versioned schema, never code; unknown security-sensitive fields are rejected rather than ignored; an imported plugin is untrusted however valid its JSON; table identity is derived by the controller from the immutable plugin identity, never chosen by the file; and every plugin-supplied string reaching rendered configuration keeps the four-layer defence. Cost, and it is the real one: the privilege boundary that D-039 described as over the horizon — a user-authored plugin creating a QuestDB table — is now on the critical path, and the product does not have it yet.                                                                        |
| D-045 | Credential bootstrap is controller-side provisioning; Telegraf stays the only production poll executor | Accepted                         | Some APIs exchange a credential for another one — PAN-OS being the proving case — and the derived secret has to be extracted, encrypted, and stored before any poll can use it. Telegraf cannot do that: it has nowhere to put a secret and no notion of renewal, and making every poll re-derive a key would turn an authentication endpoint into a polling target. So the controller performs the bootstrap request, extracts the secret server-side, encrypts it immediately, and never persists or logs the raw response; the credential record owns expiry and renewal. Production polling is unchanged and still runs entirely through rendered Telegraf configuration, which is what D-036 actually protects. Cost: the controller makes an outbound request to a monitored device, which it has never done. That is the largest new attack surface in the feature, and every SSRF, redirect, TLS, and DNS-rebinding restriction in the plan exists because of it. The same applies to an authoring test, which runs through a short-lived isolated Telegraf candidate precisely so the preview exercises the path production will use rather than a second one.                                                                                                                                                                                                                                                                        |
| D-043 | The accessibility and WCAG 2.2 AA audit is dropped from scope, not deferred                            | Accepted                         | Owner's decision, 2026-08-11. Recorded as dropped rather than left open, because an item that stays on a list nobody intends to do makes every other open item mean less. What exists is not nothing: every screen is measured at 320, 360, and 390px by `apps/web/tests/e2e/responsive.spec.ts`, pages carry a visually hidden `<h1>` for landmark structure, destructive actions go through a described dialog rather than `window.confirm` (D-029), and form controls are labelled — but none of that was verified against the standard, and keyboard traversal, focus order, contrast ratios, and reduced-motion handling were never measured at all. Cost: the product cannot claim WCAG 2.2 AA conformance, and should not imply it. Anyone procuring against an accessibility requirement — which is most of the public sector, and a growing share of everyone else — will need this done before they can buy. Reopen it as its own piece of work if that day comes, rather than pretending the gap is smaller than it is.                                                                                                                                                                                                                                                                                                                                                                                                             |
...
Task-based usability testing is what keeps the version at `0.1.x`. A `1.0.0`
would claim it was done.

The **accessibility and WCAG 2.2 AA audit was dropped from scope** by the owner
on 2026-08-11 (D-043) rather than deferred, so it is not counted above and will
not reappear as an open item.
...
**One item remains open**, and it is not code: task-based usability testing.
The accessibility and WCAG 2.2 AA audit was dropped from scope on 2026-08-11
(D-043), not deferred. Milestone 8 is not complete until the usability testing
is done, and that is what keeps the version at `0.1.x`.
...
| ------------------- | ------------------------------------------------------------------------------------------------------------------------- | ------------------------------ |
| 1. Pluggable inputs | Any measurement, over any transport, attached to a source as a chip — written, imported, and exported as declarative JSON | Not started, planned in detail |
...
a code change. A user can also **write** an input, test it against a device they
already have, and hand the result to someone else as a file.
...
### Authoring, import, and export

Plugins are portable, versioned JSON documents. A plugin file contains declarative
collection behaviour only; it never contains credentials, API keys, device
addresses, executable code, shell commands, JavaScript, or arbitrary Telegraf
configuration.

A plugin may be created through the interface, uploaded from a local JSON file,
downloaded as a canonical JSON file, tested against an existing device before
installation, and installed into the catalogue after validation.

A downloaded file must be sufficient to recreate the plugin on another
installation, and must never include credentials, generated secrets, test
responses, device identifiers, or environment-specific state.

#### Manifest lifecycle

- [ ] Give every manifest a schema version, immutable plugin identity, plugin
      version, display name, description, transport, response format, credential
      requirements, polling constraints, requests, and declared measurements.
- [ ] Validate uploaded files against a strict versioned JSON schema before
      storing them.
- [ ] Reject unknown security-sensitive fields rather than silently ignoring them.
- [ ] Apply limits to file size, request count, header count, selector length,
      preview size, and measurement count.
- [ ] Store the canonical manifest plus its digest, origin, author when known,
      installation time, and validation status.
- [ ] Define identity and version collision behaviour. Uploading a different
      document with an existing identity and version must fail rather than
      silently replace it.
- [ ] Preserve older plugin versions while checks still refer to them.
- [ ] Never mutate an installed plugin version. A changed manifest creates a new
      version and goes through measurement migration checks.
- [ ] Allow download of the canonical stored manifest, so export and import are
      deterministic.
- [ ] Distinguish built-in, locally authored, and imported plugins in the
      catalogue.
- [ ] Do not treat an imported plugin as trusted merely because its JSON schema is
      valid.

#### Authoring workflow

The authoring interface is a guided manifest builder, not a raw
collector-configuration editor. The flow: choose a transport and response format;
define credential requirements; select an existing device and compatible
credential for a test; configure and execute a bounded preview request through the
same collection path production uses; inspect a redacted and truncated preview;
select values to retain; give every selected measurement a stable key, display
name, type, unit, semantic kind, and optional entity identity; validate; save as a
new plugin version or download as JSON; attach to one or more devices as checks.

Test execution must use a short-lived, isolated Telegraf candidate generated by
the controller. The controller may orchestrate and display the test, but it must
not become an alternative production collector. That preserves D-036 and proves
the generated production configuration is executable — a preview that took a
different path would prove nothing about the thing that will actually run.

A successful preview does not install or attach the plugin.

#### SNMP authoring

The user chooses scalar or table collection, enters numeric OIDs, selects an
SNMP-capable device and credential, executes a bounded test, sees the returned
values — and for tables the discovered rows and columns — selects what becomes a
measurement, and confirms the type and unit of each.

- [ ] Validate numeric OID syntax and enforce maximum OID and result counts.
- [ ] Do not accept MIB names unless the server resolves them deterministically to
      numeric OIDs.
- [ ] Represent scalar and table collection explicitly in the manifest.
- [ ] For table measurements, define stable entity keys separately from measured
      fields.
- [ ] Show instance OIDs in the preview, without accidentally encoding one
      device's transient instance identifiers as the universal plugin schema.
- [ ] Require a declared type and unit. A previewed value may suggest a type; it
      cannot establish a semantic unit.
- [ ] Permit marking returned fields as entity tags, measurements, or ignored.
- [ ] Detect duplicate measurement keys and incompatible types before
      installation.
- [ ] Ensure SNMP test failures do not reveal community strings, authentication
      material, or privacy keys.

#### HTTP authoring

HTTP plugins support bounded declarative workflows in two phases: an optional
**credential bootstrap**, executed when a credential is created, refreshed, or
explicitly tested; and one or more **poll requests**, executed by Telegraf on the
configured schedule.

The manifest is not a general workflow engine. It may declare HTTP methods,
relative paths, query parameters, headers, body encoding, response format,
selectors, and references to explicitly defined secret slots. It may not declare
loops, arbitrary code, unrestricted templates, filesystem access, redirects to
arbitrary hosts, or shell execution.

Response formats must include at least JSON through constrained selectors, XML
through constrained XPath selectors, and GraphQL responses treated as HTTP JSON
after GraphQL error handling.

A plugin-supplied parser expression is untrusted input. XML parsing must disable
external entities and network access.

#### Authentication modes

None; HTTP Basic; bearer token; static custom header; query-parameter secret only
where a target API requires it; and a derived API key created by a credential
bootstrap request.

Plugins declare named credential slots and where those values may be used. Actual
values live only in encrypted credential records. A plugin file must never contain
a username, password, bearer token, session cookie, API key, custom-header value,
or preview-derived secret.

#### Credential bootstrap

Some APIs exchange one credential for another. That is credential provisioning,
not an ordinary measurement request, and the manifest models it separately.

A bootstrap definition declares the HTTP method and relative endpoint, request
content type, placement of input credential fields, response format, the selector
for the derived secret, the target secret slot, an optional expiry selector,
permitted success statuses, and redacted error handling.

During preview, secret-looking values must not be selectable as ordinary
measurements. The author explicitly marks one response field as the derived
secret; the controller extracts it server-side, encrypts it immediately, and
thereafter exposes only whether the value is present and when it expires. The raw
bootstrap response is never persisted or logged.

If the derived secret expires, the credential record owns its renewal state.
Polling must not generate a fresh secret on every interval.

#### The Palo Alto Networks proving case

PAN-OS is the first derived-key proving case: a bootstrap POST of administrator
username and password to the key-generation endpoint; the returned API key
selected from the XML response **as a secret, not as a measurement**; stored as a
generated-key credential; used by Telegraf for subsequent polling, through the
`X-PAN-KEY` header where supported; a poll request invoking the XML operational
command for global counters; the preview presenting counter names and values; and
the author selecting which become measurements with declared type and unit.

It proves form-encoded POST bootstrap, XML response parsing, XPath secret
extraction, derived credential storage, secret injection into a later request, XML
operational responses, and dynamic measurement selection — in one case.

The API key must not appear in an example URL, query string, fixture, log,
screenshot, manifest, or documentation. Use a header where supported.

#### Request restrictions

- [ ] Derive the request origin from the attached device. Manifests may contain
      relative paths, never arbitrary production hosts.
- [ ] Prevent SSRF to loopback, link-local, metadata, control-plane, and otherwise
      prohibited addresses.
- [ ] Revalidate the resolved destination on every connection, against DNS
      rebinding.
- [ ] Do not follow redirects to a different origin.
- [ ] Require TLS verification by default.
- [ ] Model private certificate authorities explicitly rather than adding a global
      "skip verification" switch.
- [ ] Allow only a fixed set of HTTP methods, content types, and body encodings.
- [ ] Reject CR/LF and other configuration-breaking characters in headers and
      interpolated fields.
- [ ] Restrict interpolation to declared device fields and credential slots.
- [ ] Apply timeouts, response-size limits, concurrency limits, and polling-rate
      constraints.
- [ ] Redact secrets from generated configuration previews, logs, audit events,
      errors, and test output.
- [ ] Validate rendered candidates with the existing four-layer defence before
      executing them.

#### Preview and measurement selection

A preview is untrusted data returned by a managed device.

- [ ] Limit preview depth, item count, byte size, and rendering time.
- [ ] Truncate large responses safely, and say the preview is incomplete.
- [ ] Escape preview content before rendering it in React.
- [ ] Never persist raw preview responses by default.
- [ ] Identify likely secrets and exclude them from ordinary measurement
      selection.
- [ ] Let the user select scalar or repeating fields from JSON or XML.
- [ ] Convert each selection into an explicit response selector stored in the
      manifest.
- [ ] Require stable measurement identity independent of display name.
- [ ] Require type and unit for every measurement.
- [ ] Allow an optional description, semantic kind, aggregation behaviour, and
      counter-versus-gauge classification.
- [ ] Validate selectors against the preview response and again during candidate
      generation.
- [ ] Define missing-value, parse-error, and incompatible-type behaviour per
      measurement.

A checkbox means "include this field in the declared measurement set". It must not
create a database column without manifest validation and an explicit confirmation
step.
...
- [ ] Replace the SNMP-only credential model with a kinded one: SNMPv2c, SNMPv3,
      HTTP basic, HTTP bearer, HTTP custom header, derived key, and none.
...
- [ ] Give a derived-key credential its own health state: present, expired, or
      failed to renew. A bootstrap failure must surface as a credential health
      error, not as collection quietly stopping.
...
- [ ] Decide and record how a plugin's HTTP response maps to measurements — a
      declared JSON path or XPath per measurement, evaluated by Telegraf's parser,
      not by code the plugin ships.
- [ ] Run authoring tests through a short-lived isolated Telegraf candidate, so
      the preview exercises the path production will use.
...
- [ ] Create no table on installation. A table appears only when an installed and
      validated plugin version becomes desired state through an attached check.
- [ ] Derive and validate table identity from the immutable plugin identity and
      version. An imported plugin must not be able to choose a table name.
- [ ] Before accepting a new version, compare its measurement schema against every
      installed version sharing that identity, classifying the change as additive,
      rename, type change, unit change, removal, semantic change, or entity-key
      change. **Automate additive changes only**; reject the rest with a
      migration-required status until a deliberate QuestDB migration design
      exists.
...
- [ ] Add "Create plugin" and "Upload plugin" to the catalogue, and "Download
      JSON" to every plugin version.
- [ ] Provide separate guided editors for SNMP and HTTP.
- [ ] Show the selected test device and credential clearly, and make plain that
      neither becomes part of the saved manifest.
- [ ] Show a redacted request preview before execution.
- [ ] Display validation errors beside the manifest field that caused them.
- [ ] Show measurement type, unit, selector, and entity behaviour before saving.
- [ ] Require confirmation before replacing a draft with an uploaded file.
- [ ] Keep the chips, preview trees, and measurement selection legible below 400px
      and reachable by keyboard — the responsive suite measures every screen and
      will fail otherwise. Keyboard reachability is stated here as a requirement
      of this feature; it is not covered by a general audit, because that audit
      was dropped from scope (D-043).
...
- [ ] Extend the security suite: a plugin manifest is new untrusted input, an
      uploaded file is a new ingress, and a credential kind is a new secret shape.
- [ ] Audit plugin creation, import, export, test, installation, and version
      change — without recording secrets or raw responses.
...
- Existing SNMP collection is unchanged in behaviour and now runs through the
  plugin path.
- A user creates an SNMP scalar plugin by entering an OID, testing it against an
  existing device, selecting the returned value, declaring its type and unit, and
  installing the result.
- A user creates an SNMP table plugin and selects measurements without binding the
  plugin to transient row instances from the test device.
- A user exports a plugin to JSON and imports it into another clean installation
  with the same manifest identity and digest. The exported file contains no device
  information and no credentials.
- An invalid, oversized, executable, or policy-violating manifest is rejected
  before persistence or test execution.
- A user creates a PAN-OS plugin that generates an API key through a POST
  bootstrap, extracts it from XML, stores it as a write-only derived credential,
  and uses it for a separate XML operational request.
- PAN-OS global counters selected during preview appear as declared measurements,
  native charts, Grafana panels, and alert metrics without application code
  changes.
- No PAN-OS password, generated API key, or raw authentication response appears in
  logs, manifests, exports, URLs, previews, or API reads.
- A derived-key failure or expiry produces a credential health error rather than
  silently stopping collection.
- Production polling still runs through rendered Telegraf configuration, not
...
Settled by the owner on 2026-08-09, before implementation rather than during it:
...
Revised on 2026-08-11, when the authoring, import, and export scope above was
adopted:

- **D-039 is superseded by D-044.** It held that the first cut ships a built-in
  catalogue only, with user-authored plugins following once the manifest had
  survived a few real ones. Upload, download, and guided creation make
  user-authored plugins reachable immediately, so the decision it rested on no
  longer holds and pretending otherwise would leave the record lying.
- **D-044** — locally authored and imported plugins are in scope from the first
  cut, with the validation and privilege boundary that requires.
- **D-045** — credential bootstrap is a bounded provisioning operation performed
  by the controller, while Telegraf remains the sole executor of production polls.

Consequences that still have no design, and should not be discovered during
implementation:

- **A plugin whose measurement set changes needs a metrics-store migration.** That
  is the price of table-per-plugin, and it is the right price, but nothing in the
  product migrates QuestDB today. Additive changes are automatable; the rest are
  blocked until that design exists.
- **A user-authored plugin creates a QuestDB table.** Under D-039 this was out of
  reach; under D-044 it is the first thing that happens. It is a privilege
  boundary the product does not have, and it is now on the critical path rather
  than over the horizon.
- **The controller makes an outbound request to a monitored device** during
  bootstrap and during an authoring test. D-045 permits it and bounds it, but
  every SSRF, redirect, TLS, and rebinding restriction listed above exists because
  of it. This is the largest new attack surface in the feature.

## Completed log

### 2026-08-09 (later still)

- Fixed a drift probe that lied. Grafana's QuestDB datasource had held a stale
  password since the D-027 rename on 2026-08-07: every Grafana panel returned
  400 while the badge said "6 managed resources are current". The password is
  excluded from the content hash the probe compares, so the one piece of state
  that had actually drifted was the one it could not see. The probe now measures
  the datasource instead (D-040), and `dataSourceHealth` reads Grafana's body on
  any status — a 400 there is the answer the endpoint exists to give, and
  treating it as a transport error turned "the password is wrong" into "Grafana
  is unreachable".
- Proved it both ways rather than asserting it: broke the password and watched
  the badge turn pending with QuestDB's own words, applied, and watched it clear.
